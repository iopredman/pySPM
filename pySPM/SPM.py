...
class SPM_image:
    ...
    def __mul__(self, b):
        ...
        if isinstance(b, SPM_image):
            New.pixels *= b.pixels
            New.channel = f"({New.channel})*{b.channel}"
        elif type(b) in [int, float]:
            New.pixels *= b
            New.channels = f"({New.channel})*{b:.2f}"
        return New
    ...
    def pxs(self):
        ...
        """
        fxy = {
            xy: funit(self.size["real"][xy], self.size["real"]["unit"]) for xy in "xy"
        }
        return [
            (fxy[xy]["value"] / self.size["pixels"][xy], fxy[xy]["unit"]) for xy in "xy"
        ]
    ...
    def add_scale(
        ...
        if isinstance(loc, int):
            ...
            assert len(loc) == 2
            ref = (
                ax.transData.transform(loc)
                + ax.transAxes.transform((-fL / 2, -fH / 2))
    ...
    def offset(self, profiles, width=1, ax=None, col="w", inline=True, **kargs):
        ...
        **kargs: arguments passed further to get_row_profile.
        ...
        if inline:
            self.pixels = self.pixels - np.flipud(
                np.repeat(offset, self.pixels.shape[1], axis=1)
            )
            return self
        else:
            C = copy.deepcopy(self)
            C.pixels = self.pixels - np.flipud(
                np.repeat(offset, self.pixels.shape[1], axis=1)
            )
            return C
    ...
    def get_row_profile(
        self, x1, y1, x2, y2, width=1, col="C1", ax=None, alpha=0, **kargs
    ):
        """
        ...
        **kargs:
        ...
        plotargs = {
            key: kargs[key]
            for key in ["linewidth", "color", "linestyle"]
            if key in kargs
        }
        if y2 < y1:
        ...
        if ax is not None:
            ...
            if kargs.get("axPixels", False):
                ...
                ax.plot(
                    [(x1 - dx) * pxs, (x1 + dx) * pxs],
                    [(h - (y1 - dy)) * pys, (h - (y1 + dy)) * pys],
                    col,
                )
                ax.plot(
                    [(x2 - dx) * pxs, (x2 + dx) * pxs],
                    [(h - (y2 - dy)) * pys, (h - (y2 + dy)) * pys],
                    col,
                )
                ax.plot(
                    (x1 * pxs, x2 * pxs),
                ...
                if kargs.get("label", False):
                    ax.annotate(
                        kargs.get("label"),
                        (0.5 * (x1 + x2) * pxs, 0.5 * (2 * h - y1 - y2) * pys),
                        color=col,
                    )
                if alpha > 0:
                    import matplotlib.patches
        ...
        I = scipy.interpolate.RectBivariateSpline(x, y, np.transpose(np.flipud(self.pixels)))
        
        Y = np.arange(y1, y2 + 1)
        V = np.zeros(len(Y))
        for w in np.arange(width):
            xl = np.linspace(
                x1 - (width - 1) / 2.0 + w, x2 - (width - 1) / 2.0 + w, len(Y)
            )
        ...
        **kargs,
        ...
        if not pixels:
            if isunit != 6:
                ...
                if u == "u":
                    u = "µ"
                ax.set_xlabel(f"x [{u}{unit}]")
                ax.set_ylabel(f"y [{u}{unit}]")
            else:
    ...
    def real2pixels(self, x, y, float=False):
        ...
        if not float:
            px = np.digitize(
                x,
                np.linspace(
                    0, self.size["real"]["x"] / (10**fact), self.pixels.shape[1]
                ),
                right=True,
            )
            py = np.digitize(
                y,
                np.linspace(
                    0, self.size["real"]["y"] / (10**fact), self.pixels.shape[0]
                ),
                right=False,
            )
        else:
    ...
    def circular_profile(
        ...
        axProfile=None,
        plotProfileEvery=1,
        xtransf=lambda x: x * 1e9,
        ytransf=lambda x: x * 1e9,
        ToFcorr=False,
        fit=lambda x, *p: p[3] + p[2] * CDF(x, *p[:2]),
        p0=None,
        errors=False,
        ...
        **kargs,
        ...
        for i, angle in enumerate(np.linspace(A, B, N)):
            ...
            if axProfile and i % plotProfileEvery == 0:
                ...
                axProfile.plot(
                    xtransf(l - p0[0]), fit(l, *p0), color=scalarMap.to_rgba(i)
                )
        # close loop
        if A % 360 == B % 360:
        ...
        if axPolar:
            axPolar.plot(
                angles,
                ytransf(res[:, 1]),
                color=kargs.get("sig_color", "C0"),
                label="$\\sigma$",
            )
            axPolar.plot(
                angles,
                ytransf(fact * res[:, 1]),
                color=kargs.get("fwhm_color", "C1"),
                label="FWHM",
            )
            if errors:
                axPolar.fill_between(
                    ...
                    ytransf(res[:, 1] + cov[:, 1]),
                    color=kargs.get("sig_color", "C0"),
                    alpha=kargs.get("fillalpha", 0.5),
                )
                axPolar.fill_between(
                    angles,
                    fact * ytransf(res[:, 1] - cov[:, 1]),
                    ytransf(res[:, 1] + cov[:, 1]),
                    color=kargs.get("fwhm_color", "C1"),
                    alpha=kargs.get("fillalpha", 0.5),
                )
    ...
    def get_profile(
        ...
        **kargs,
        ...
        if not pixels:
            if kargs.get("debug", False):
                print(
                    "Image range (real scale):",
                    self.size["real"]["x"] / (10**fact),
            ...
            if not axPixels:
                xvalues, p = get_profile(
                    np.flipud(self.pixels),
                    x1,
                    y1,
                    x2,
                    y2,
    ...
    def plot_profile(
        ...
        **kargs,
        ...
        ax.set_xlabel(f"Distance [{u}{unit}]")
        if zunit is not None:
            ax.set_ylabel(f"{self.channel} [{zunit}]")
        else:
            ax.set_ylabel(f"{self.channel} [{self.zscale}]")

        return {"plot": Plot, "l": xvalues, "z": profile}

    def get_bin_threshold(
        self, percent, high=True, adaptive=False, binary=True, img=False
    ):
        """
    ...
    def filter_gaussian(self, sigma, inline=True, **kwargs):
        ...
        """
        if inline:
            self.pixels = scipy.ndimage.gaussian_filter(
                self.pixels, sigma=sigma, **kwargs
            )
        else:
            C = copy.deepcopy(self)
...
def beam_profile(
    ...
    **kargs,
    ...
    return np.fft.fftshift(
        real(np.fft.ifft2(np.fft.fft2(target) * recon_tf))
    ) / np.size(target)
...
def get_profile(
    ...
    **kargs,
    ...
    for w in np.linspace(-width / 2, width / 2, max(1, width)):
        ...
        M = scipy.ndimage.interpolation.map_coordinates(
            I, np.vstack((y, x)), order=interp_order
        )
        P.append(M)
    if kargs.get("debug", False):
        print("get_profile input coordinates:", x1, y1, x2, y2)
...
def generate_k_matrices(A, dx, dy):
    ...
    k[
        0, 0
    ] = 1.0  # Prevent division by zero error and illegal operand errors. This may be improved...
    return k, kx, ky
        ...
        if transform != 0:
            ...
            tf_out *= (
                (
                    ...
                    + 1j
                    * (
                        np.cos(phi) * np.sin(-theta) * kx
                        + np.sin(phi) * np.sin(-theta) * ky
                    )
                )
                / k
            ) ** transform
...
def get_tik_tf(Img, mu, tukey=0, source_tukey=0, debug=False, d=200, real=np.real):
    ...
    popt, pcov = scipy.optimize.curve_fit(
        fit, np.arange(len(P)), P, p0, bounds=((0, 0, -np.inf, 0), np.inf)
    )
    bg = popt[2]
    a = popt[0]
    if debug:
